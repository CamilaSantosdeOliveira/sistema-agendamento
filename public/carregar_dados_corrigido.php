...
foreach ($professores as $prof) {
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            ...
            echo "<div class='error'>❌ Erro ao inserir professor {$prof[0]}: " . $stmt->error . "</div>";
        }
        $stmt->close();
    } else {
        echo "<div class='error'>❌ Erro na preparação: " . $conn->error . "</div>";
    }
}
...
foreach ($alunos as $aluno) {
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            ...
            echo "<div class='error'>❌ Erro ao inserir aluno {$aluno[0]}: " . $stmt->error . "</div>";
        }
        $stmt->close();
    } else {
        echo "<div class='error'>❌ Erro na preparação: " . $conn->error . "</div>";
...
foreach ($cursos as $curso) {
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            ...
            echo "<div class='error'>❌ Erro ao inserir curso {$curso[0]}: " . $stmt->error . "</div>";
        }
        $stmt->close();
    } else {
        echo "<div class='error'>❌ Erro na preparação: " . $conn->error . "</div>";
