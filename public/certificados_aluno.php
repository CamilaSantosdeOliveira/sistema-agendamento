...
// Buscar certificados do aluno (simulados baseados em cursos concluídos)
...
// Buscar estatísticas
$total_certificados = $certificados->num_rows;
$certificados_concluidos = 0;
$certificados_em_andamento = 0;

// Calcular estatísticas
$certificados_array = [];
while ($cert = $certificados->fetch_assoc()) {
    $certificados_array[] = $cert;
    if ($cert['aulas_concluidas'] >= 3) { // Considera concluído se fez pelo menos 3 aulas
        $certificados_concluidos++;
    } else {
        $certificados_em_andamento++;
    }
}
?>
<!DOCTYPE html>
...
<body>
    ...
    <div class="dashboard-container">
            ...
            <nav class="sidebar-nav">
                <ul class="sidebar-menu">
                    ...
                    <div class="sidebar-group">
                        ...
                        <li class="sidebar-item">
                            <a href="certificados_aluno.php" class="sidebar-link active">
        ...
        <main class="main-content">
            <header class="header">
                <h1><i class="fas fa-certificate"></i> Certificados</h1>
            ...
            <div class="stats-grid">
                <div class="stat-card">
                    ...
                    <div class="value"><?php echo $total_certificados; ?></div>
                    <div class="label">Total de Cursos</div>
                </div>
                <div class="stat-card">
                    <div class="icon">
                        <i class="fas fa-check-circle"></i>
                    </div>
                    <div class="value"><?php echo $certificados_concluidos; ?></div>
                    <div class="label">Certificados Concluídos</div>
                </div>
                <div class="stat-card">
                    <div class="icon">
                        <i class="fas fa-clock"></i>
                    </div>
                    <div class="value"><?php echo $certificados_em_andamento; ?></div>
                    <div class="label">Em Andamento</div>
                </div>
            </div>

            <!-- Certificates -->
            <?php if (count($certificados_array) > 0): ?>
                <div class="certificate-grid">
                    <?php foreach ($certificados_array as $cert): ?>
                        <?php 
                        $progresso = $cert['total_aulas'] > 0 ? ($cert['aulas_concluidas'] / $cert['total_aulas']) * 100 : 0;
                        $is_completed = $cert['aulas_concluidas'] >= 3; // Considera concluído se fez pelo menos 3 aulas
                        $status_class = $is_completed ? 'status-completed' : 'status-progress';
                        $status_text = $is_completed ? 'Concluído' : 'Em Andamento';
                        ?>
                        <div class="certificate-card">
                            <div class="certificate-header">
                                <div>
                                    <div class="certificate-title"><?php echo $cert['nome']; ?></div>
                                    <div class="certificate-category"><?php echo $cert['categoria']; ?></div>
                                </div>
                                <span class="certificate-status <?php echo $status_class; ?>">
                                    <i class="fas fa-<?php echo $is_completed ? 'check-circle' : 'clock'; ?>" style="font-size: 0.7rem;"></i>
                            ...
                            <div class="certificate-details">
                                <div class="detail-item">
                                    ...
                                    <span><?php echo $cert['nivel']; ?></span>
                                </div>
                                <div class="detail-item">
                                    <i class="fas fa-clock"></i>
                                    <span><?php echo $cert['duracao_horas']; ?>h</span>
                                </div>
                                <div class="detail-item">
                                    <i class="fas fa-check-circle"></i>
                                    <span><?php echo $cert['aulas_concluidas']; ?> de <?php echo $cert['total_aulas']; ?> aulas</span>
                                </div>
                                <div class="detail-item">
                                    <i class="fas fa-percentage"></i>
                                    <span><?php echo round($progresso); ?>% concluído</span>
                                </div>
                            </div>
                            ...
                            <div class="certificate-actions">
                                <?php if ($is_completed): ?>
                                    <button class="btn btn-primary" onclick="baixarCertificado(<?php echo $cert['id']; ?>)">
                                        <i class="fas fa-download"></i> Baixar Certificado
                                    </button>
                                <?php else: ?>
                                    <button class="btn btn-disabled" disabled>
                                        <i class="fas fa-lock"></i> Em Andamento
                                    </button>
                                <?php endif; ?>
                                <a href="detalhes_curso_aluno.php?id=<?php echo $cert['id']; ?>" class="btn btn-secondary">
            ...
            <?php else: ?>
                <div class="empty-state">
                    ...
                    <p>Você ainda não tem certificados. Complete cursos para obter seus certificados!</p>
    ...
    <script>
        ...
        function baixarCertificado(cursoId) {
            ...
            showToast('Download iniciado', 'Seu certificado está sendo preparado. (Funcionalidade simulada)', 'info');
            
            // Aqui você pode implementar a lógica real de download
            // Por exemplo, gerar um PDF do certificado e fazer o download
            console.log('Baixando certificado do curso ID:', cursoId);
        }
    </script>
